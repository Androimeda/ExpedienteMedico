...
SELECT
  ...
  EX.ID_EXPEDIENTE,
  CE.ID_MEDICO,
  (SELECT P_NOMBRE ||' '|| S_NOMBRE  ||' '||  P_APELLIDO||' '|| S_APELLIDO FROM VISTAMEDICO v WHERE v.ID_MEDICO = CE.ID_MEDICO) as medico,
  (SELECT ESPECIALIDAD FROM VISTAMEDICO v WHERE v.ID_MEDICO = CE.ID_MEDICO) as especialidad,
  CM.NOMBRE as centro_medico,
  CM.ID_TIPO_CENTRO,
  TC.DESCRIPCION AS TIPO_CENTRO,
  PE.P_NOMBRE,
...
/*** ------------------- VISTA CONSULTORIO ------------------------ *****/
CREATE OR REPLACE VIEW VistaConsultorioMedico
AS
SELECT
    C.*,
    M.ID_MEDICO,
...
SELECT
  ...
  (SELECT COUNT(*) FROM PISO p WHERE p.ID_EDIFICIO = E.ID_EDIFICIO) as total_pisos
FROM  EDIFICIO E
INNER JOIN CENTROMEDICO CM
  ON CM.ID_CENTRO_MEDICO= E.ID_CENTRO_MEDICO
INNER JOIN TIPOCENTRO TC
  ON TC.ID_TIPO_CENTRO= CM.ID_TIPO_CENTRO
ORDER BY total_pisos
;
